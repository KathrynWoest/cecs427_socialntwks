...
def neighborhood_overlap(graph, node1, node2):
    ...
    Output: the neighborhood overlap value"""

    # check to ensure the desired nodes exist in the graph
    if not graph.has_node(node1) or not graph.has_node(node2):
        raise Exception(f"Graph does not contain node '{node1}' and/or '{node2}', so neighborhood overlap can't be calculated. Program terminated.")
