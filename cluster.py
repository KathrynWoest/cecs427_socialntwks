...
def clustering_coefficient(graph, node):
    ...
    Output: the clustering coefficient of the node"""

    # check to ensure the desired node exists in the graph
    if not graph.has_node(node):
        raise Exception(f"Graph does not contain node '{node}', so clustering coefficient can't be calculated. Program terminated.")
